...
# ==========================================
APP_TITLE = "Clone Hero Chart 2 Midi"
if os.name == 'nt':
    import ctypes
...
if not is_valid_dir:
    ...
    # 4. Generate the config file for Notepad editing
...
# Exact reverse mappings from your previous script
PITCH_MAP = {
...
def convert_chart_to_midi(chart_path, midi_path):
    ...
    # Read the text .chart file into memory chunks
    try:
    ...
    # Extract Resolution (defaults to 192 if missing)
    resolution = 192
    ...
    for section_name, lines in sections.items():
        ...
        diff = None
        inst_key = None
        # Break apart difficulty and instrument strings
        ...
        for line in lines:
            ...
            if data[0] == 'N':
                color = int(data[1])
                # THE FIX: Enforce a 1-tick minimum to prevent 0-length note inversion
                length = max(int(data[2]), 1) 
                
                pitch = PITCH_MAP[diff].get(color)
                if pitch:
                    # Priority 1 for note_on, priority 0 for note_off (ensures clean sorting)
                ...
                # THE FIX: Applied to Star Power phrases as well
                length = max(int(data[2]), 1)
    ...
    for t_name, events in track_events.items():
        ...
        # Sort absolute tick. If ticks match perfectly, process note_off before note_on
